...
const USER_AGENTS = [
  ...
  "Mozilla/5.0 (Xiaomi; MI 12) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Mobile Safari/537.36",
];
...
/**
 * Web scraping utilities for Xiaomi Mi Store (GetApps)
 */
export const xiaomiWebScraper = {
  async fetchTopCharts({
    ...
    try {
      const baseUrl = `https://app.mi.com`;
      ...
      console.error(`[XiaomiWebScraper] Error fetching top charts:`, error);
      return [];
    }
  },
  ...
  async fetchCategoryTop({
    ...
    try {
      ...
      console.error(`[XiaomiWebScraper] Error fetching category top:`, error);
      return [];
    }
  },
  ...
  async fetchAppDetails(appId: string, country: string): Promise<any> {
    try {
      ...
      const raw = {
        ...
        summary: $(".app-summary").first().text().trim(),
      ...
      console.error(`[XiaomiWebScraper] Error fetching app details:`, error);
      throw error;
    }
  },
};
